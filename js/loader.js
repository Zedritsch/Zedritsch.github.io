const A=["de"];async function b(c){
	...
	if(document.documentElement.getAttribute("lang")==null)b("");
	if(location.pathname=="/")location.pathname="/home";
	fetch("/html"+location.pathname).then(d=>{
		if(d.ok)d.text;
		else{location.pathname="/404";
		rel()}
	}).then(e=>{document.getElementsByTagName("main")[0].innerHTML=e;b(location.pathname)});scrollTo({behavior:"smooth",top:0/*TODO*/})}onload=c